...
export default function Quiz() {
  ...
  const saveToSupabase = async () => {
    ...
    if (slidesError) {
      ...
      alert("Quiz and slides saved to Supabase!");
    }
  };
    ...
    <div className={`min-h-screen p-4 transition-colors duration-300 ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-black'}`}>
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" onClick={saveToSupabase}>Save to Supabase</Button>
            ...
            <Card>
              <CardContent className="space-y-4 p-4">
                <div>
                  ...
                  <Input type="color" value={currentSlide.background} onChange={(e) => updateSlide("background", e.target.value)} />
                </div>
                <div>
                  <label className="block mb-1 text-sm font-medium">Text Color</label>
                  <Input type="color" value={currentSlide.textColor} onChange={(e) => updateSlide("textColor", e.target.value)} />
                </div>
                <div>
                ...
                <div>
                  <label className="block mb-1 text-sm font-medium">Timer (in seconds)</label>
                  <Input type="number" min={0} value={currentSlide.timer} onChange={(e) => updateSlide("timer", parseInt(e.target.value))} className="w-full" />
                </div>
              </CardContent>
            </Card>
          </div>
