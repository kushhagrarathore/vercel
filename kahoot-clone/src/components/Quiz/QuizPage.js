  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-blue-50 to-purple-100 font-sans transition-all duration-500 px-2 sm:px-4">
      ...
      {quizPhase === 'question' && currentQuestion && timeLeft > 0 && selectedAnswer === null && (
        <div 
          className="w-full max-w-4xl bg-white rounded-2xl shadow-xl overflow-hidden animate-fade-in transition-all duration-500"
          style={{
            width: '800px',
            height: '600px',
            maxWidth: '90vw',
            maxHeight: '70vh',
            backgroundColor: currentQuestion.settings?.backgroundColor || '#ffffff',
            color: currentQuestion.settings?.textColor || '#000000',
            fontFamily: currentQuestion.settings?.fontFamily || 'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',
            fontSize: `${currentQuestion.settings?.fontSize || 20}px`,
          }}
        >
            ...
            <div className="space-y-4">
              {currentQuestion.options.map((option, index) => (
                <button
                  ...
                  className="w-full p-4 rounded-lg border-2 border-gray-200 transition-all duration-200 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-blue-300 active:scale-95"
                  style={{
                    backgroundColor: currentQuestion.settings?.buttonColor || '#3b82f6',
                    borderRadius: `${currentQuestion.settings?.buttonRadius || 8}px`,
                    color: '#ffffff',
