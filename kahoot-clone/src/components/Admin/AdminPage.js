  ...
  return (
    ...
    >
      <div className={`w-full ${presentationMode ? 'h-full flex flex-col justify-center items-center' : 'max-w-4xl mx-auto p-2 sm:p-6'}`}
        style={presentationMode ? { maxWidth: '100vw', maxHeight: '100vh', padding: 0 } : {}}>
        ...
        !session ? (
          ...
          <div className="space-y-8">
            ...
            <>
            {/* Presentation Mode: Only show quiz code, question number, and Next button at the top */}
            {presentationMode ? (
              <div className="flex flex-col w-full h-full items-center justify-center bg-gradient-to-br from-blue-50 to-purple-100 p-0 m-0 overflow-hidden" style={{ minHeight: '100vh', minWidth: '100vw' }}>
                {/* Safe-area container for 16:9, with extra top/bottom padding to prevent overflow */}
                <div className="flex flex-col w-full h-full items-center justify-center px-4 pt-8 pb-8 md:px-16 md:pt-12 md:pb-12 box-border" style={{height: '100vh', minHeight: '100vh'}}>
                  {/* Top bar with code, question number, exit button */}
                  <div className="flex flex-row w-full max-w-screen-lg justify-between items-center mb-8 pt-2 md:pt-4">
                    <span className="text-2xl md:text-3xl font-bold text-blue-700 tracking-wider">Quiz Code: <span className="text-gray-800">{session.code}</span></span>
                    <span className="text-xl md:text-2xl font-semibold text-gray-700">Question: <span className="text-blue-700">{currentQuestionIndex + 1} / {questions.length}</span></span>
                  ...
                  <div className="flex-1 flex flex-col justify-center items-center w-full h-full max-w-screen-lg mx-auto overflow-visible">
                    <div className="w-full max-w-3xl mx-auto bg-white/90 rounded-3xl shadow-2xl p-6 md:p-10 flex flex-col items-center justify-center min-h-[40vh] md:min-h-[50vh] overflow-visible">
                      <h3 className="font-bold text-2xl md:text-3xl text-gray-900 mb-8 text-center break-words">{currentQuestion?.question_text}</h3>
                      <ul className="w-full flex flex-col gap-4 md:gap-6">
                        {currentQuestion?.options.map((option, index) => (
                          <li key={index} className={`w-full`}>
                            <div className={`w-full px-0 sm:px-4`}>
                              <div className={`w-full flex items-center justify-between`}> 
                                <span className={`text-lg md:text-2xl font-medium text-gray-800 ${showCorrect && index === currentQuestion.correct_answer_index ? 'text-green-600 font-bold' : ''}`}>{option}</span>
                                {showCorrect && index === currentQuestion.correct_answer_index && <span className="ml-4 text-green-600 font-bold text-2xl">✓</span>}
                              </div>
                            </div>
                          </li>
                        ))}
                      ...
                      {quizPhase === 'question' && timeLeft > 0 && (
                        <div className="mt-10 text-2xl md:text-3xl font-bold text-purple-700 flex items-center gap-3 bg-white/80 px-8 py-3 rounded-full shadow-lg w-fit mx-auto">
                      ...
                      {quizPhase === 'question' && timeLeft === 0 && (
                        <div className="mt-10 text-green-700 font-bold text-2xl">Time's up! Correct answer shown above.</div>
                      )}
                      {showCorrect && pollResults.length > 0 && (
                        <div className="mt-10 w-full">
                          <h4 className="font-bold mb-4 text-xl text-center">Poll Results</h4>
                          <div className="space-y-3">
                            {currentQuestion.options.map((option, idx) => {
                              ...
                              return (
                                <div key={idx} className={`flex items-center ${idx === currentQuestion.correct_answer_index ? 'bg-green-100' : 'bg-gray-100'} rounded-xl p-3 w-full`}>
                                  <span className="flex-1 text-lg sm:text-xl">{option}{idx === currentQuestion.correct_answer_index && <span className="ml-2 text-green-600 font-bold">✓</span>}</span>
                                  <div className="w-1/2 mx-2 bg-gray-200 rounded h-5 relative">
                                    <div
                                      className={`h-5 rounded ${idx === currentQuestion.correct_answer_index ? 'bg-green-400' : 'bg-blue-400'}`}
                                      style={{ width: `${percent}%` }}
                                    ></div>
                      ...
                      {/* Always show Next/Start button at the bottom in Presentation Mode */}
                      <div className="flex justify-center mt-12 w-full">
                        {quizPhase === 'question' && (
                          <button
                            onClick={nextQuestion}
                            className="px-10 py-4 bg-blue-600 text-white rounded-2xl font-bold text-2xl shadow-lg hover:bg-blue-700 transition-all"
                        ...
                        {quizPhase === 'waiting' && (
                          <button
                            onClick={startQuestion}
                            className="px-10 py-4 bg-green-600 text-white rounded-2xl font-bold text-2xl shadow-lg hover:bg-green-700 transition-all"
                            style={{ minWidth: '180px' }}
                          >
                            Start
                          </button>
